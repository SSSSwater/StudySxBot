...
#     PRIVATE
from nonebot.adapters.qq import Bot, MessageEvent,Message,MessageSegment,GroupAtMessageCreateEvent,Event
...
# 群权限
async def group_rule(event: GroupAtMessageCreateEvent):
    group_list = study_config.dict()["study_group"]
    if str(event.group_openid) in group_list:
        return True
    else:
        return False
...
test = on_command("test", aliases=set(study_config.test_key), permission=group_rule, priority=50)
daka = on_command("daka", aliases=set(study_config.daka_key), priority=50)
reg = on_command("reg", aliases=set(study_config.reg_key), permission=group_rule, priority=50)
reg_list = on_command("reg_list", aliases=set(study_config.reg_list_key), permission=group_rule, priority=50)
modify_name = on_command("modify_name", aliases=set(study_config.modify_name_key), permission=group_rule,
                         priority=50)
today_list = on_command("today_list", aliases=set(study_config.today_list_key), permission=group_rule,
                        priority=50)
remind_once = on_command("remind_once", aliases=set(study_config.remind_once_key), permission=group_rule,
                         priority=50)
remind_once_cancel = on_command("remind_once_cancel", aliases=set(study_config.remind_once_cancel_key),
                                permission=group_rule, priority=50)
...
@daka.handle()
async def _daka(bot: Bot, event: GroupAtMessageCreateEvent, state: T_State, args: Message = CommandArg()):
    print(event.json())
    state['word_num'] = 0
    pic_url = None
    for a in args:
...
async def _daka_img(bot:Bot, event: GroupAtMessageCreateEvent, state: T_State):
    print(state)
